...
def eval():
    ...
    output_file = f'test_predict/oracle_predict_tmp.csv'
    output_cmat = f'test_predict/oracle_cmat_tmp.csv'
    #output_file = f'test_predict/{model}_predict_lr-3_fixt.csv'
    #output_cmat = f'test_predict/{model}_cmat_lr-3_fixt.csv'

    cv_metric = np.zeros((5,4))

    for i in range(5):

        #result_nn = f'test_result/nn3c_test_cv-{i+1}_lr-3.csv'
        result_B = f'test_result/MIL2B_test_cv-{i+1}_lr-3_fixt.csv'
        result_T = f'test_result/MIL2T_test_cv-{i+1}_lr-3_fixt.csv'
        result_H = f'test_result/MIL2H_test_cv-{i+1}_lr-3_fixt.csv'

        #slide_id_tmp, slide_prob_tmp, _, pred_label_tmp, max_prob_tmp = get_slide_prob_label(result_nn, 3)
        slide_id_tmp, slide_prob_B, true_label_tmp, pred_label_B, _ = get_slide_prob_label(result_B, 2)
        _, slide_prob_T, _, pred_label_T, _ = get_slide_prob_label(result_T, 2)
        _, slide_prob_H, _, pred_label_H, _ = get_slide_prob_label(result_H, 2)

        np_true = np.array(true_label_tmp)

        #np_nn = np.array(pred_label_tmp)
        np_6c = np.zeros((len(slide_id_tmp)))
        for j in range(len(slide_id_tmp)):
            if np_true[j] == 0 or np_true[j] == 1:
                np_6c[j] = pred_label_B[j]
            elif np_true[j] == 2 or np_true[j] == 3:
                np_6c[j] = pred_label_T[j] + 2
            elif np_true[j] == 4 or np_true[j] == 5:
                np_6c[j] = pred_label_H[j] + 4

        slide_prob_6c = np.zeros((len(slide_id_tmp),6))

        for j in range(len(slide_id_tmp)):
            if np_true[j] == 0 or np_true[j] == 1:
                slide_prob_6c[j,0] = slide_prob_B[j][0]
                slide_prob_6c[j,1] = slide_prob_B[j][1]
            elif np_true[j] == 2 or np_true[j] == 3:
                slide_prob_6c[j,2] = slide_prob_T[j][0]
                slide_prob_6c[j,3] = slide_prob_T[j][1]
            elif np_true[j] == 4 or np_true[j] == 5:
                slide_prob_6c[j,4] = slide_prob_H[j][0]
                slide_prob_6c[j,5] = slide_prob_H[j][1]

        AUC_class = np.zeros((6))
        ...
        cv_metric[i,0] = accuracy_score(np_true, np_6c)
        cv_metric[i,1] = precision_score(np_true, np_6c, average='macro')
        cv_metric[i,2] = recall_score(np_true, np_6c, average='macro')
