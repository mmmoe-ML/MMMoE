...
def train_model(rank, world_size, cv_test):
    ...
    list_subtype = [['AITL'],['ATLL']]
    list_use = ['AITL','ATLL']#,'CHL','RL']
    ...
    # log file
    log = f'./train_log/MIL2T_log_cv-{cv_test}.csv'
    ...
    # create model
    from model import feature_extractor, MLP_attention, fc_label, fc_sub, MIL, MIL2
    #
    feature_extractor6c = feature_extractor()
    MLP_attention6c = MLP_attention(2048, 512)
    fc_label6c = fc_label(2048, 512, n_class=6)
    #
    MIL6c = MIL(feature_extractor6c, MLP_attention6c, fc_label6c, 6)
    MIL6c.load_state_dict(torch.load(model_params,map_location='cpu'))

    feature_extractor2c = MIL6c.feature_ex # use pre-trained feature extractor
    MLP_attention2c = MLP_attention(512, 128)
    fc_sub = fc_sub()
    fc_label2c = fc_label(512, 128, n_class=n_class)
    MIL2c = MIL2(feature_extractor2c, MLP_attention2c, fc_sub, fc_label2c, n_class)
    for param in MIL2c.feature_ex.parameters():
        param.requires_grad = False
    MIL2c = MIL2c.to(rank)
    process_group = torch.distributed.new_group([i for i in range(world_size)])
    #
    MIL2c = nn.SyncBatchNorm.convert_sync_batchnorm(MIL2c, process_group)
    #
    ddp_model = DDP(MIL2c, device_ids=[rank])
    ...
    for epoch in range(EPOCHS):
        ...
        if rank == 0:
            model_params = f'./model_params/MIL2T_cv-{cv_test}_epoch-{epoch}.pth'
