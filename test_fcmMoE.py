...
def test_model(cv_test, t, params_fix):
    ...
    if params_fix == 0:
        ...
        result = f'./test_result/fcmMoE_test_cv-{cv_test}_t-{t}.csv'
    else:
        log = f'./train_log/fcmMoE_log_cv-{cv_test}_t-{t}_fix.csv'
        result = f'./test_result/fcmMoE_test_cv-{cv_test}_t-{t}_fix.csv'

    epoch_m = select_epoch(log)

    if params_fix == 0:
        model_params = f'./model_params/fcmMoE_cv-{cv_test}_epoch-{epoch_m}_t-{t}.pth'
    else:
        model_params = f'./model_params/fcmMoE_cv-{cv_test}_epoch-{epoch_m}_t-{t}_fix.pth'
    ...
    # create model
    from model import feature_extractor, MLP_attention, fc_sub, fc_label, MLP_fcmgating, fcmMoE
    #
    ...
    fc_gating = MLP_fcmgating (3)
    MoE = fcmMoE(feature_extractor, fc_subB, fc_subT, fc_subH, fc_gating, MLP_attention, fc_label, n_class=n_class, t=t)
