        ...
        for bag_num in range(input_tensor.shape[0]):
            with torch.no_grad():
                with torch.cuda.amp.autocast(enabled=True):
                    class_prob, class_hat, g_w, att = model(input_tensor[bag_num])
...
def test_model(cv_test, t, params_fix):
    ...
    #
    if params_fix == 0:
        log = f'./train_log/imgMoE_log_cv-{cv_test}_t-{t}.csv'
        result = f'./test_result/imgMoE_test_cv-{cv_test}_t-{t}.csv'
    else:
        log = f'./train_log/imgMoE_log_cv-{cv_test}_t-{t}_fix.csv'
        result = f'./test_result/imgMoE_test_cv-{cv_test}_t-{t}_fix.csv'

    epoch_m = select_epoch(log)

    if params_fix == 0:
        model_params = f'./model_params/imgMoE_cv-{cv_test}_epoch-{epoch_m}_t-{t}.pth'
    else:
        model_params = f'./model_params/imgMoE_cv-{cv_test}_epoch-{epoch_m}_t-{t}_fix.pth'
    ...
    # create model
    from model import feature_extractor, MLP_attention, fc_sub, fc_label, MLP_gating, imgMoE
    #
    ...
    fc_gating = MLP_gating(3)
    MoE = ImgMoE(feature_extractor, fc_subB, fc_subT, fc_subH, fc_gating, MLP_attention, fc_label, n_class=n_class, t=t)
