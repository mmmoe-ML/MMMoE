...
def train_model(rank, world_size, cv_test, t, params_fix):
    ...
    # create model
    from model import feature_extractor, MLP_attention, fc_label, fc_sub, MLP_mmgating, MIL2, mmMoE, fc_FCM
    #
    ...
    MLP_gatingMoE = MLP_mmgating (fc_fcmMoE, 3)

    MLP_attentionMoE = MLP_attention(512, 128) #
    fc_labelMoE = fc_label(512, 128, n_class=n_class) #
    MoE = mmMoE(feature_extractorMoE, fc_subMoEB, fc_subMoET, fc_subMoEH, MLP_gatingMoE, MLP_attentionMoE, fc_labelMoE, n_class=n_class, t=t)
    ...
    for epoch in range(EPOCHS):
        ...
        if rank == 0:
            if params_fix == 0:
                model_params = f'./model_params/mmMoE_cv-{cv_test}_epoch-{epoch}_t-{t}.pth'
            else:
                model_params = f'./model_params/mmMoE_cv-{cv_test}_epoch-{epoch}_t-{t}_fix.pth'
