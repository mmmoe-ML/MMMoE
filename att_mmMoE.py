    ...
    for (input_tensor, slideID, class_label, fcm_tensor, pos_list) in test_loader:
        ...
        for bag_num in range(input_tensor.shape[0]):
            ...
            for i in range(len(pos_list)):
                ...
                tmp.append(float(g_w[i,0]))
                tmp.append(float(g_w[i,1]))
                tmp.append(float(g_w[i,2]))
...
def test_model(cv_test, t, params_fix):
    ...
    for slideID in test_all:
        ...
        test_dataset.append([slideID, class_label, data_all[list_id.index(slideID),5:23].astype(np.float32)])
        i += 1

    # output
    if params_fix == 0:
        log = f'./train_log/mmMoE_log_cv-{cv_test}_t-{t}.csv'
        makedir(f'mmMoE_att_t-{t}')
        result = f'mmMoE_att_t-{t}'
    else:
        log = f'./train_log/mmMoE_log_cv-{cv_test}_t-{t}_fix.csv'
        makedir(f'mmMoE_att_t-{t}_fix')
        result = f'mmMoE_att_t-{t}_fix'
