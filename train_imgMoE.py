...
def train_model(rank, world_size, cv_test, t, params_fix):
    ...
    # create model
    from model import feature_extractor, MLP_attention, fc_label, fc_sub, MLP_gating, imgMoE, MIL2
    #
    ...
    MLP_gatingMoE = MLP_gating(3) #
    MoE = imgMoE(feature_extractorMoE, fc_subMoEB, fc_subMoET, fc_subMoEH, MLP_gatingMoE, MLP_attentionMoE, fc_labelMoE, n_class=n_class, t=t)
    ...
    for epoch in range(EPOCHS):
        ...
        if rank == 0:
            if params_fix == 0:
                model_params = f'./model_params/imgMoE_cv-{cv_test}_epoch-{epoch}_t-{t}.pth'
            else:
                model_params = f'./model_params/imgMoE_cv-{cv_test}_epoch-{epoch}_t-{t}_fix.pth'
